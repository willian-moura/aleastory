module.exports = function createGame(){
    ...
    function submitWord(command){
        ...
        if (!state.players[player].submitted){
            ...
            return {status: false, msg: `Player ${player} already submitted a word!`};
        }
    }
    ...
    function voteWord(command){
        ...
        if (!state.players[player].voted){
            ...
            if (word.player == player){
                ...
                return {status: false, msg: `${player}, you can't vote in your own word!`};
            }
            word.votes += 1;
        ...
        if (state.gameDuration > 0){
            ...
            if (state.stage === 0){
                ...
                if (state.stageClock > 0){
                    ...
                    state.currentText += state.lastBestWord.word!='' ? `${state.lastBestWord.word} ` : '';
                    ...
                    for (const p in state.players)
                        state.players[p].voted = false;
                    for (const p in state.players)
                        state.players[p].submitted = false;
